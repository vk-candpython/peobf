...
// [UINT32=total_len:4|BYTE=key_size:1|LPCBYTE=key:key_size|UINT32=raw_len:4|LPCBYTE=encrypted_exe]
    PBYTE PE_DATA = ReadOverlay();
    if (!PE_DATA) Exit(STATUS_DATA_ERROR);
    


    const UINT32 PE_DATA_SZ = *(const UINT32*)PE_DATA;
    const BYTE   PE_KEY_SZ  = *(PE_DATA + 4);
    const UINT32 PE_EXE_SZ  = *(const UINT32*)(PE_DATA + 4 + 1 + PE_KEY_SZ);

    const LPCBYTE
        PE_KEY = (LPCBYTE)(PE_DATA + 4 + 1),
        PE_EXE = (LPCBYTE)(PE_DATA + 4 + 1 + PE_KEY_SZ + 4);



    PVOID   buf = NULL;
    {SIZE_T bsz = PE_EXE_SZ;
    ...
    UnPack(
        (PBYTE)buf, 
        PE_EXE, 
        PE_DATA_SZ - (4 + 1 + PE_KEY_SZ + 4), 
        PE_EXE_SZ, 
        PE_KEY,
        PE_KEY_SZ - 1
    ); FreeBuf(PE_DATA, PE_DATA_SZ);}
